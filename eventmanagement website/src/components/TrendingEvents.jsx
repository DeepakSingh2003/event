...
const formatPrice = (price) => {
  ...
  if (numberPrice >= 1000) {
    return `${(numberPrice / 1000).toFixed(numberPrice % 1000 === 0 ? 0 : 1)}k`;
  }
    ...
    const fetchFeaturedEvents = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/events?featured=true&country=India`,
        );
  ...
  return (
    <div className="bg-white p-6 md:p-10 font-sans max-w-screen-2xl mx-auto">
      ...
      {!loading && !error && events.length === 0 && (
        <p className="text-gray-500 font-medium">No featured events found.</p>
      )}
